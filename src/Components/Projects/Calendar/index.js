...
export default function Calendar() {
	...
	return (
		<>
			<Mold
				title="Calendario"
				description="SE PLANEJE VENDO A DATA DO ANO, PROCURE POR MEIO DE FILTROS, ETC."
			/>
			<Container className="container">
				<div className="calendar-assets">
					<h1 id="currentDate"></h1>
					<div className="field">
						<label htmlFor="date">Pesquisa por data</label>
						<form
							className="form-input"
							id="date-search"
							onSubmit={setDate(this)}
						>
							<input
								type="date"
								...
								required
							/>
							<button type="submit" className="btn btn-small" title="Pesquisar">
								<i className="fas fa-search"></i>
							</button>
						</form>
					</div>
					<div className="day-assets">
						<button className="btn" onClick={prevDay()} title="Dia anterior">
							<i className="fas fa-chevron-left"></i>{" "}
						</button>
						<button className="btn" onClick={resetDate()} title="Dia atual">
							<i className="fas fa-calendar-day"></i> Hoje
						</button>
						<button className="btn" onClick={nextDay()} title="Próximo dia">
							<i className="fas fa-chevron-right"></i>{" "}
						</button>
					</div>
				</div>
				<div className="calendar" id="table">
					<div className="header">
						<div className="month" id="month-header"></div>
